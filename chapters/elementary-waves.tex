...
\begin{definition}
  ...
  is arbitrary, and will be convenient later. It is also convenient to define $b_0=0$, to avoid constantly treating the $a_n$ coefficients separately for having an $n=0$ coefficient.
\end{definition}
...
In all this chapter, we discussed elementary waves and their combinations as functions of a single real variable.
However, oscillatory physical systems will often require multiple variables to be described. We already encountered that in~\cref{chap:wave-eq} when studying the wave equation. We found that the wave equation solution~\cref{eq:string-fourier} for the plucked string was a periodic function both in the time and space dimensions. Before starting to discuss how this result can help us to define trigonometric polynomials in multiple dimensions, we need to formulate a definition for multidimensional periodic functions. This can be done simply by considering functions that are periodic in each of their variables:
\begin{definition}
  A function $f$ on $\mathbb{R}^d$ is said to be \emph{periodic} or specifically \emph{$\vv{\tau}$-periodic} if there exists a vector $\vv{\tau}$ with positive components such that $f(\vv{x})=f(x_1,\dots,x_d)$ is $\tau_j$-periodic in the variable $x_j$, for all directions $j$. The vector $\vv{\tau}$ is called a \emph{period} of $f$, and the vector $\vv{\omega}$ defined by
  \begin{equation}
    \omega_j=\frac{1}{\tau_j}\,,
  \end{equation}
  is called a \emph{frequency} of $f$.
...
We could inspire ourselves from the solution~\cref{eq:string-fourier} of the plucked string problem to define trigonometric polynomials in $d$ dimensions to be linear combinations of products of $d$ elementary sine or cosine waves. Although this would be a fine definition, it would need a tedious discussion of the combinatorics of all the possible independent cross-terms between sine and cosine waves. This is, once more, a situation where using complex exponentials brings considerable simplifications, since the product of complex waves is still a complex wave. Explicitly, assuming a vector of $d$ real variables $\vv{x}=(x_1,\dots,x_d)$, and a vector of $d$ frequencies $\vv{\omega}=(\omega_1,\dots,\omega_d)$, we can write
\begin{align}
  e^{2\pi i\omega_1x_1}\cdots e^{2\pi i\omega_dx_d}&=e^{2\pi i(\omega_1x_1+\cdots+\omega_dx_d)}=e^{2\pi i\vv{\omega}\cdotp\vv{x}}\notag\\
  &=\cos(2\pi\vv{\omega}\cdot\vv{x})+i\sin(2\pi\vv{\omega}\cdot\vv{x})\,.
\end{align}
This motivates the following definitions for multidimensional elementary waves
\begin{definition}
  We call \emph{elementary sine wave}, \emph{elementary cosine wave}, and \emph{elementary complex wave} on $\mathbb{R}^d$ the functions defined by
  \begin{align}
    \sw(\vv{x};\vv{\omega},\phi)&=\sin(2\pi\vv{\omega}\cdot\vv{x}+\phi)\,,\\
    \cw(\vv{x};\vv{\omega},\phi)&=\cos(2\pi\vv{\omega}\cdot\vv{x}+\phi)\,,\\
    \ew(\vv{x};\vv{\omega},\phi)&=e^{2\pi i\vv{\omega}\cdotp\vv{x}+\phi}\,,
  \end{align}
  respectively, where the \emph{frequency} $\vv{\omega}\in\mathbb{R}^d$ has only non-zero components, and the \emph{phase} $\phi$ is a real number such that $0\leq \phi <2\pi$.
\end{definition}
Because multidimensional elementary waves are essentially defined as one-dimensional waves applied on a dot product, most identities from the previous section can be easily generalised. The previous statement can be investigated further in the following way: for a given frequency vector $\vv{\omega}$, any vector $\vv{x}$ can be uniquely written as
\begin{equation}
  \vv{x}=\vv{x}_{\parallel}+\vv{x}_{\perp}\,,
\end{equation}
where $\vv{x}_{\parallel}$ is the orthogonal projection of $\vv{x}$ on $\vv{\omega}$, and $\vv{x}_{\perp}$ is the perpendicular remainder. Explicitly
\begin{equation}
  \vv{x}_{\parallel}=(\vv{x}\cdot\hat{\vv{\omega}})\,\hat{\vv{\omega}}
  \qquad\text{and}\qquad
  \vv{x}_{\perp}=\vv{x}-\vv{x}_{\parallel}\,,
\end{equation}
where $\hat{\vv{\omega}}$ is the unit vector in the direction of $\vv{\omega}$, \ie
\begin{equation}
  \hat{\vv{\omega}}=\frac{\vv{\omega}}{|\vv{\omega}|}\,.
\end{equation}
One can check explicitly that $\vv{x}_{\perp}$ is indeed orthogonal to $\vv{\omega}$:
\begin{equation}
  \vv{x}_{\perp}\cdot\vv{\omega}
  =\vv{x}\cdot\vv{\omega}-\vv{x}_{\parallel}\cdot\vv{\omega}
  =|\vv{\omega}|(\vv{x}\cdot\hat{\vv{\omega}})-(\vv{x}\cdot\hat{\vv{\omega}})(\hat{\vv{\omega}}\cdot\vv{\omega})=0\,,
\end{equation}
and therefore, using sine waves as an example
\begin{equation}
  \sw(\vv{x};\vv{\omega},\phi)=\sin(2\pi\vv{\omega}\cdot\vv{x}_{\parallel}+\phi)\,.
\end{equation}
So the oscillations of a sine wave with frequency vector $\vv{\omega}$ only occur in the direction parallel to $\vv{\omega}$, and the wave is constant in the perpendicular direction. Assuming $\vv{x}$ is aligned with $\vv{\omega}$, \ie it can be written $\vv{x}=z\hat{\vv{\omega}}$ with $z\in\mathbb{R}$, then
\begin{equation}
  \sw(\vv{x};\vv{\omega},\phi)=\sin(2\pi|\vv{\omega}|z+\phi)=\sw(z;|\vv{\omega}|,\phi)\,.
\end{equation}
In summary, $\sw(\vv{x};\vv{\omega},\phi)$ is a one-dimensional sine-wave of frequency
$|\vv{\omega}|$ on the axis defined by $\hat{\vv{\omega}}$, and is constant in all
perpendicular directions. In three dimensions, the perpendicular space to the frequency
vector is the plane with equation $\vv{\omega}\cdot\vv{x}=0$, and for that reason the
waves defined above are generally called \emph{plane waves}. An example of two-dimensional plane wave is given in~\cref{fig:plane-wave}.
\begin{figure}[t]
  \centering
  \includegraphics{gp_wave-plane.pdf}
  \caption{The left pane represents the plane wave $\sw(\vv{x};\vv{\omega})$, with $\vv{x}=(x,y)$ and $\vv{\omega}=(1,2)$. The arrows represent the unit vectors $\hat{\bm{\omega}}=(1,2)/\sqrt{5}$, and $\hat{\bm{\omega}}_{\perp}=(2,-1)/\sqrt{5}$ in the perpendicular direction. The right pane shows the interference pattern emerging when adding two sine plane waves with frequency vectors $(1,2)$ and $(1,-1)$.}
  \label{fig:plane-wave}
\end{figure}
\begin{figure}[t]
  \centering
  \includegraphics{gp_wave-sphere.pdf}
  \caption{Representation of the apherical wave...}
