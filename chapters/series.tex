...
\begin{corollary}
  \label{corr:rl}
  Let $f$ be a function on $\R$ piecewise smooth on a compact interval $[a,b]$. We
...
\begin{proof}
  ...
  \end{equation}
  and since $|\sin(bk)-\sin(ak)|<2$ for all $k\in\R$, the integral above vanishes
...
generally imply uniform convergence. We know that uniform convergence implies pointwise convergence
...
% First an important consequence of the results of~\cref{sec:series-l2-conv} is that the
% Fourier expansion of a function is unique.
% \begin{proposition}
%   Let $f$ be $\tau$-periodic piecewise smooth function. Let $\gamma_n$ be a sequence of
%   complex numbers such that
%   \begin{equation}
%     f(t)=\sumz{n}\gamma_n\,e^{2\pi i n\omega t}\,,
%   \end{equation}
%   for $t\in\R$ such that $t$ is not a jump discontinuity of $f$. Then the $\gamma_n$ are the
%   Fourier coefficients of $f$, \ie for all $n\in\Z$,
%   \begin{equation}
%     \gamma_n=c_n(f)\,.
%   \end{equation}
% \end{proposition}
