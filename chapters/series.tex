...
\begin{definition}
	...
	$\tau$-periodic function is said to be piecewise smooth if it is piecewise smooth on
	a period.
\end{definition}
...
\begin{proof}
	...
	\begin{align}
		\int_a^b\diff t\, f(t)\,g'(t)
		 & =f(t_1^-)g(t_1)-f(a^+)g(a)+f(t_2^-)g(t_2)
		-f(t_1^+)g(t-1)+\dots\notag                                                 \\
...
\begin{corollary}
	\label{cor:rl}
	Let $f$ be a function on $\R$ piecewise smooth on a compact interval $[a,b]$. We have
...
\begin{proof}
	...
	\end{equation}
	and since $|\sin(bk)-\sin(ak)|<2$ for all $k\in\R$, the integral above vanishes for
	$k\to+\infty$. A similar argument can be made for the complex exponential, although one
	can also notice that it is now a trivial consequence the equality
	\begin{equation}
...
\begin{proof}
	...
	\begin{equation}
		...
		=\frac12\frac{\sin[(N+\frac{1}{2})x]}{\sin(\frac{x}{2})}+\frac 12\,.
	\end{equation}
...
\begin{proof}
	...
	\begin{equation}
		s_N(f)(t)=\frac{1}{\tau}\int_0^\tau\diff u\,f(u)\sum_{n=-N}^{N}e^{2\pi i
				n\omega (t-u)}\,.
	\end{equation}
...
convergence, and conclude the section with the Gibbs phenomenon, which describes
important practical consequences of Fourier series which fail to converge uniformly.
%-----------------------------------------------------------------------------------------
\subsection{Pointwise convergence}
The theorem below can be considered as the main result of this chapter. It states that
for piecewise smooth periodic functions, the Fourier series converges at every point to
the average of the left and right limits. The essence of this theorem is that the Fourier
series does converge back to a given piecewise smooth periodic function, expect perhaps
at a finite number of points of jump discontinuity.
\begin{theorem}
...
We now generalise some results of~\cref{sec:ew-orth}, which we will then use to prove
that Fourier series generally converge in mean square. Firstly, it is not too difficult
to show that the norm of a partial Fourier sum is always smaller than the norm of the
associated function, an important result known as \emph{Bessel's inequality}.
...
\begin{proof}
	...
	proves the theorem.
\end{proof}
In reality, this inequality is an equality, generalising~\cref{thm:trigp-parseval}. This
...
We now discuss uniform convergence, which is the strongest possible form of convergence
for Fourier series.
%-----------------------------------------------------------------------------------------
...
\begin{definition}
	...
	\begin{equation}
		\forall n\geq N,\quad\forall x\in
		D,\quad|f_n(x)-f(x)|<\epsilon\,,\label{eq:uniconv-eps}
	\end{equation}
...
\begin{theorem}
	...
	\begin{equation}
		\lim_{x\to x_0}\lim_{n\to+\infty}f_n(x)=\lim_{n\to+\infty}\lim_{x\to
			x_0}f_n(x)=f(x_0)\,,
	\end{equation}
	for all $x_0\in D$.
\end{theorem}
...
\end{proof}
Another important result is the exchange of limit and integral:
\begin{theorem}
...
\end{theorem}
The proof of the theorem above requires to know the Fourier coefficients of the derivative
...
\begin{proof}
	...
	\begin{align}
		...
		-\frac{1}{\tau}\sum_{m=1}^{N-1}h_m\,e^{-2\pi im\omega t_m}
		+\frac{2\pi i n\omega}{\tau}\int_0^\tau\diff t\,f(t)\,e^{-2\pi i n\omega t} \notag \\
		        & =\frac{1}{\tau}[f(\tau^-)-f(0^+)]
		-\frac{1}{\tau}\sum_{m=1}^{N-1}h_m\,e^{-2\pi im\omega t_m}
...
\begin{proof}[Proof of \cref{thm:fourier-uniconv}]
...
convergence do not generally imply uniform convergence. We know that uniform convergence
...
If a periodic function has at least one jump discontinuity, then its Fourier series
cannot be uniformly convergent on $\R$. This generates irregularities around
...
\begin{figure}[t]
	\centering
	\includegraphics{tikz_ven-periodic.pdf}
...
\begin{proposition}
	...
	\end{enumerate}
\end{proposition}
\begin{proof} Let us demonstrate the properties for the complex coefficients. When
	...
	\begin{enumerate}
		...
		\item Time translation: we have
		      \begin{align}
			      c_n(\mathcal{T}_af)
			       & =\frac{1}{\tau}\int_0^{\tau}\diff t\, f(t-a)\,e^{-2\pi i n\omega t}        \\
		...
		\item Frequency translation: we have
		      \begin{align}
			      c_{n-m}(f)
			       & =\frac{1}{\tau}\int_0^{\tau}\diff t\, f(t)\,e^{-2\pi i (n-m)\omega t} \\
...
\begin{proof}
	...
	obtain
	\begin{equation}
		c_n^\tau(f)=e^{-2\pi i n\omega \tau_0}c_n^\tau(f)=e^{-2\pi i
				\frac{n}{r}}c_n^\tau(f)\,.
	\end{equation}
...
\begin{definition}
	...
	\begin{align}
		c_{\vec{n}}(f) & =\frac{1}{V(\vec{\tau})}\braket{f,\ew(\vec{\omega}\odot\vec{n})}\notag \\
		               & =\frac{1}{V(\vec{\tau})}
		\int_{P(\vec{\tau})}\diff^d\vec{x}\,e^{-2\pi
				i(x_1n_1\omega_1+\cdots+x_dn_d\omega_d)}\,,
	\end{align}
...
\begin{theorem}
	...
	\end{equation}
\end{theorem}
For functions which are more generally square-integrable, we also have a multidimensional
...
convergence for discontinuous function and the Gibbs phenomenon do also happen in multiple
...
\begin{figure}[t]
	\centering
	\includegraphics{gp_bessel-J01.pdf}
	\caption{Curves of the Bessel functions $J_0$ and $J_1$.}
	\label{fig:bessel-J01}
\end{figure}
\begin{figure}[t]
	\centering
	\includegraphics{gp_disk-indicator.pdf}
