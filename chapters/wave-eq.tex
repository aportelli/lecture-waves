...
\begin{equation}
  ...
  \begin{cases}
    ...
    -\frac12u_0(2nL-x)&~\text{if}\quad(2n-1)L\leq x\leq 2nL
  \end{cases}
  \,,\label{eq:f-copies}
\end{equation}
...
\begin{figure}[t]
  ...
  \caption{Graphical representation of~\cref{eq:f-copies}. The bold red line represents
    half of the initial string position $u_0(x)$. The blue line is the forward component $f(x)$,
  obtained as the periodic extension of $\frac12 u_0(x)$ defined in~\cref{eq:f-copies}.}
  \label{fig:string-f}
\end{figure}
\begin{figure}[t]
  \centering
...
\end{equation}
then the extension~\cref{eq:f-copies} of $u_0$ to $f$ is defined, up to a factor $2$, by the same formula, \ie
$f(x)=\smash{\frac{A}{2}\sin\left(\frac{\pi}{L}x\right)}$ for any real number $x$, and the solution
