...
\end{proof}
% \begin{proposition} Let $f$ be a Schwartz function on $\R$, the following properties
%   hold \begin{enumerate} \item If $f$ is real, the for all $\omega\in\R$,
%   $\hat{f}(-\omega)=\hat{f}(\omega)^*$ \end{enumerate} \end{proposition}
...
\begin{corollary}
  ...
  successive Fourier transforms cancel each others. Additionally, if $f$ and $g$ are two
  Schwartz functions,
  \begin{equation}
