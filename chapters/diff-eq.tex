...
\end{equation}
The wave equation for $\hat{u}$ then become
\begin{equation}
...
of $\omega$. Therefore, there exists two constants $A(\omega)$ and $B(\omega)$ such that
\begin{equation}
  \hat{u}(\omega,x)=A(\omega)\cos\left(2\pi\frac{\omega}{c}x\right)+
  B(\omega)\sin\left(2\pi\frac{\omega}{c}x\right)\,.
\end{equation}
Let us now apply boundary conditions \cref{eq:string-bc-2}, clearly
...
sine function above cancels if and only if  $\omega=\frac{cn}{2L}$ for $n\in\Z$, so we
...
to makes physical sense. Using an initial condition $u(0,x)=u_0(x)$, we obtain
\begin{equation}
...
\begin{equation}
  ...
  \,.
\end{equation}
Since $u_0$ represent the initial string position, we can safely assume it is a smooth
...
becomes
\begin{equation}
  4\pi^2(c^2\omega_x^2-\omega_t^2)\hat{u}(\omega_t,\omega_x)=
  \frac{1}{\mu}\hat{F}(\omega_t,\omega_x)\,.
\end{equation}
So, at least formally, we found the particular solution
\begin{equation}
...
\end{equation}
There is a clear issue in the integral above, as the integrand is singular on the
...
example by adding a small damping term, however there is no unique way to do that. The
