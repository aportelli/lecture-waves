...
\begin{itemize}
  \item If $a>0$, $P(\omega)$ has two real single roots $\omega_{\pm}=\pm\frac{\sqrt{a}}{2\pi}$;
...
\cref{thm:structure-poly} regardless: since $\omega=0$ is a double root of $P(\omega)$,
there exist two constants $A$ and $B$ such that
\begin{equation}
...
Finally, if $a<0$, $P(\omega)$ has no real roots and, strictly speaking, the only
...
two complex roots of $P(\omega)$, which leads to the potential solution
\begin{equation}
