...
\begin{ExerciseList}
  %---------------------------------------------------------------------------------------
  \Exercise[label=ft-trans] Prove~\cref{prop:ft-trans}. For the \emph{differentiation in
    frequency} property, we assume we can use the \emph{Leibniz integral rule}, which can be
  formulated as follows. Let $F(x,\omega)$ be a function of two real variables such that
  $|F(x,\omega)|$ is integrable, $|\smash{\pd{}{\omega}F(x,\omega)}|$ exists everywhere
  and is integrable, then the function defined
  by
  \begin{equation}
